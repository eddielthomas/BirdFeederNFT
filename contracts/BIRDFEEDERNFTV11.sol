    ...
    Pausable
{
    ...
    uint256 public minimumMintPrice = 1 ether; // Mint price
    ...
    mapping(string => address) private referralCodes;
    mapping(address => string[]) private ownerReferralCodes;
    mapping(address => uint256) public discounts;
    mapping(address => uint256) private referralCodesGenerated;
    ...
    function batchMint(
        ...
        require(
            msg.value >= (minimumMintPrice * data.length),
    ...
    function generateReferralCode(uint256 tokenId) external {
        ...
        ownerReferralCodes[msg.sender].push(referralCode);

        referralCodesGenerated[msg.sender]++;
        emit ReferralCodeGenerated(msg.sender, tokenId, referralCode);
    }

    function getMyReferralCodes() external view returns (string[] memory) {
        return ownerReferralCodes[msg.sender];
    }
